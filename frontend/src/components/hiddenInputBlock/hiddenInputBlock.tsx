...
export function HiddenInputBlock ({value, isEdit, isWarning, onEnter, onDelete, onEdit}:HiddenInputBlockProps) {
  ...
  return (
    <div onMouseEnter={()=>{!isEdit && setIsHover(true)}} onMouseLeave={()=>{!isEdit && setIsHover(false)}} className={styles.hiddenInput}>
      {isEdit ? 
        ...
        : 
        <div onClick={onEdit} className={styles.hiddenInput__line}>
          <p>{value}</p>
          {isHover && <div className={styles.hiddenInput__icons}>
            <Icon onClick={onEdit} glyph='edit' glyphColor='black' size={16}/>
